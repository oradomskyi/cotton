...
void BYPASS::start_read()
  ...
  void BYPASS::handle_read(const boost::system::error_code& error, std::size_t n)
  {
    if (stopped_)
      return;
...
void BYPASS::start_write()
  ...
  void BYPASS::handle_write(const boost::system::error_code& error)
  {
    if (stopped_)
      return;
  ...
  void BYPASS::updateOutputBuffer(string rawPathQS, string body)
  {
